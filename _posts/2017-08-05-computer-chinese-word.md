...
参与过物联网开发的项目，笔者做服务端，跟硬件设备通信时，对接数据的工作上，认识到了嵌入式设备或者使用大端方式存储数据，或者因为大端的数据比较容易调试(从低地址到高地址打出来的数据以ASCII格式看，就是发送数据的样子)，另外，网络传输的字节也是大端方式，而x86架构的计算机是以小端方式存储数据。开发时亲自写过从硬件的大端数据到x86服务器可读的小端数据的相互转化的代码。但之后过了一段时间，笔者再次回忆大小端的区别的时候，发觉`记不住`大小端的区别了！
